    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          error.message?.includes("job_cards")
        ) {
      ...
      console.log("[v0] Caught error in loadIncompleteJobs:", error.message)
      if (
        error.message?.includes("job_cards") ||
        error.message?.includes("schema cache") ||
        error.message?.includes("Could not find the table")
        ...
        console.error("Unexpected error loading incomplete jobs:", error)
        setHasError(true)
      }
    } finally {
      setIsLoading(false)
